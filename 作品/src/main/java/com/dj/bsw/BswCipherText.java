...
public class BswCipherText implements Serializable{

	public Element C; //C=h^s
	public Element Cs; //Cs = Me(g1, g2)^(as)
	public HashMap<String, Element[]> Cys = new HashMap<>(); //(leafNode->(Cy, Cy')), map leafNode to (Cy, Cy') which Cy is g1^(gy(0)) and Cy' is H(att(y))^(qy(0))
	 ...
	 */
	void generateMap(BswCipherText cipherText, BswNode root, BswPub pub) {
		if(root.n == 0) {
