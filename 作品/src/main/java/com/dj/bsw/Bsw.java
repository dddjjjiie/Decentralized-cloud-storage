...
public class Bsw {
	 ...
	 */
	public static BswCipherText encrypt(BswPub pub, Element m, BswAccessTree accessTree) {
...
//		cipherText.accessTree = accessTree;
	 ...
	 * @param sk
	 */
	public static Element decrypt(BswCipherText cipherText, BswSk sk, BswAccessTree accessTree) {
		Element cs = cipherText.Cs.duplicate();
		Element e_c_d = BswPub.pair.pairing(cipherText.C, sk.D);
		decryptNode(null, accessTree.root, BswPub.pair.getZr().newElement().setToOne(), 0, sk, cipherText);
