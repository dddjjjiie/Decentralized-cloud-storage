...
function postOrder(root){
  if(root.isParent){
      for(var i=0; i<root.children.length; i++){
          console.log("-----", root.name, "-----", i, root.children.length);
...
function upload() {
  ...
  filesOk.forEach(function(file){
    ...
    reader.onloadend = function() {
      ...
      // response.push(uploadCount)
      document.querySelector("#response").innerText = response
      updateList(fileChecksum(file), uploadCount)
      if (uploadCount == filesOk.length) {
        document.querySelector('.min-loading.blue').classList.add('loading-hidden');  //stop loading event
        // document.querySelector('button#buttonRemote').removeAttribute('disabled', '')
        // document.querySelector('button#buttonLocal').removeAttribute('disabled', '')
...
function resetFiles() {
  ...
  document.querySelector("pre#response").innerHTML =  '<spam id="info">Response:</spam>'
